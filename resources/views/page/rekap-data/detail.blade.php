...
@section('content')
    ...
    <div class="min-h-screen py-6 md:py-10 text-[#FF3EA5] font-sans selection:bg-[#FF3EA5] selection:text-white print:hidden"
                ...
                <div class="bg-white border-b-4 border-[#FF3EA5] p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="min-w-0">
                            ...
                            <h2 class="text-xl md:text-3xl font-black uppercase tracking-tighter leading-tight truncate">{{ $rekap->doctor->name }}</h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                        ...
                        <div class="space-y-8">
                            ...
                            <div>
                                ...
                                <div class="p-5 md:p-6 bg-pink-50 border-2 md:border-4 border-[#FF3EA5] rounded-[1.5rem] font-bold text-sm md:text-base leading-relaxed text-[#FF3EA5] shadow-[4px_4px_0px_0px_#FF3EA5]">
                            ...
                            {{-- Diagnosa --}}
                            <div>
                                <div class="inline-block bg-[#FF3EA5] text-white px-3 py-1 rounded-lg text-[9px] md:text-[10px] font-black uppercase tracking-widest mb-3">Diagnosa</div>
                            ...
                            {{-- Resep --}}
                            <div>
                                <div class="inline-block bg-white text-[#FF3EA5] border-2 border-[#FF3EA5] px-3 py-1 rounded-lg text-[9px] md:text-[10px] font-black uppercase tracking-widest mb-3">Resep & Saran</div>
                                <div class="p-5 md:p-6 bg-white border-2 md:border-4 border-[#FF3EA5] border-dashed rounded-[1.5rem] font-bold text-sm md:text-base leading-relaxed whitespace-pre-line">{{ $rekap->prescription }}</div>
                            </div>
                        </div>

                        {{-- Kolom Kanan: Foto --}}
                        <div>
                            <div class="inline-block bg-[#FF3EA5] text-white px-3 py-1 rounded-lg text-[9px] md:text-[10px] font-black uppercase tracking-widest mb-3">Hasil Foto USG</div>
                            @if ($rekap->image)
        ...
        {{-- Modal Lightbox --}}
        <div x-show="showModal" x-transition.opacity class="fixed inset-0 z-[100] bg-white/90 backdrop-blur-sm flex items-center justify-center p-4 md:p-10" style="display: none;">
            <div class="relative bg-white border-4 border-[#FF3EA5] rounded-[2rem] md:rounded-[3rem] p-2 shadow-[12px_12px_0px_0px_#FF3EA5] max-w-5xl w-full" @click.away="showModal = false">
                <button @click="showModal = false" class="absolute -top-4 -right-4 md:-top-6 md:-right-6 w-10 h-10 md:w-14 md:h-14 bg-white border-4 border-[#FF3EA5] text-[#FF3EA5] font-black text-lg md:text-2xl rounded-full shadow-[4px_4px_0px_0px_#FF3EA5] hover:rotate-90 transition-all">X</button>
    ...
    <div id="print-template" class="hidden print:block print:bg-white text-black font-serif p-8">
        ...
        <div class="border-b-4 border-black pb-4 mb-6 flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                {{-- 1. Logo (Ganti path gambarnya sesuai file kamu) --}}
                {{-- Saya kasih class 'grayscale' biar pas diprint warnanya jadi hitam putih (formal) --}}
                <img src="{{ asset('images/logo-icon.png') }}" class="w-16 h-16 object-contain grayscale" alt="Logo Mamacare">
        ...
        {{-- BAGIAN KELUHAN (Variable sudah diganti jadi $rekap) --}}
        <div class="mb-6">
        ...
        <div class="border-2 border-black p-0 mb-8">
            ...
            <div class="p-4">
                ...
                <p class="text-lg leading-relaxed whitespace-pre-line font-mono">{{ $rekap->prescription ?: 'Tidak ada resep khusus.' }}</p>
            </div>
        </div>

        {{-- LAMPIRAN FOTO USG (Sudah Rata Kiri / justify-start) --}}
        @if($rekap->image)
        <div class="border-2 border-black p-4 mb-8">
            <p class="font-bold uppercase text-xs mb-4">Lampiran Foto USG:</p>
            {{-- Perubahan disini: justify-start --}}
            <div class="flex justify-start bg-gray-50 border border-gray-300 p-2">
                <img src="{{ asset('storage/' . $rekap->image) }}" class="max-w-full h-auto max-h-[300px] object-contain">
            </div>
        </div>
        @else
        ...
        {{-- FOOTER --}}
        <div class="mt-8 text-center text-[10px] text-gray-400 italic">Dokumen ini dihasilkan secara digital oleh sistem Mamacare.</div>
    </div>
@endsection