...
<div class="min-h-screen bg-white text-[#FF3EA5] py-8">
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        {{-- Header Page --}}
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6 border-b-2 border-[#FF3EA5] pb-8">
            <div>
                ...
                <p class="text-xs font-bold mt-2 opacity-60 uppercase tracking-wider">Konsultasi Full-Screen dengan Ahlinya.</p>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            @foreach($doctors as $doc)
                <div class="bg-white border-2 border-[#FF3EA5] rounded-[2rem] p-6 shadow-[6px_6px_0px_0px_#FF3EA5] hover:shadow-none hover:translate-x-[3px] hover:translate-y-[3px] transition-all flex flex-col h-full group">
                    
                    {{-- Profile Info --}}
                    <div class="flex items-center gap-5 mb-6">
                        <div class="w-16 h-16 rounded-full border-2 border-[#FF3EA5] p-1 bg-white shrink-0 shadow-sm">
                            {{-- Avatar Style --}}
                            <div class="w-full h-full bg-pink-50 rounded-full flex items-center justify-center text-[#FF3EA5] font-black text-2xl uppercase border-2 border-[#FF3EA5]/10">
                                {{ $doc['avatar'] }}
                            </div>
                        </div>
                        <div class="min-w-0">
                            <h4 class="font-black text-lg uppercase leading-tight truncate text-[#FF3EA5]">{{ $doc['name'] }}</h4>
                            <div class="flex items-center gap-1.5 mt-1">
                                <span class="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
                                <p class="text-[10px] font-black opacity-60 uppercase tracking-tighter">{{ $doc['specialist'] }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-auto grid grid-cols-5 gap-3">
                        ...
                        {{-- Tombol WhatsApp --}}
                        <a href="https://wa.me/?text=Halo%20Dokter%20{{ $doc['name'] }}" target="_blank"
