...
<style>
    ...
    .chat-container-neo {
        ...
        overflow: hidden; /* Mencegah container ini meluber */
        position: relative;
    }
    ...
    #chat-scroll-area {
        ...
        overflow-y: auto;
        padding: 20px;
        background-color: #fdf2f8; /* Background chat */
        display: flex;
        flex-direction: column;
        gap: 16px;
    }
    ...
    #chat-scroll-area::-webkit-scrollbar-thumb { background: #FF3EA5; border-radius: 10px; }

    /* Bubble Chat Lebar */
    .bubble-chat {
        word-wrap: break-word;
        max-width: 75%; /* Supaya tidak terlalu rapat ke pinggir lawan */
        line-height: 1.5;
    }
...
<div class="chat-container-neo" x-data="chatSystem()" x-init="initChat({{ $activeDoctor->id }})">
    ...
    <div class="h-16 md:h-20 bg-white border-b-4 border-[#FF3EA5] flex items-center justify-between px-6 shrink-0 z-20">
        <div class="flex items-center gap-4 min-w-0">
            <a href="{{ route('mama.tanya-dokter') }}" class="text-[#FF3EA5] bg-white border-2 border-[#FF3EA5] p-2 rounded-xl shadow-[3px_3px_0px_0px_#FF3EA5] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none transition-all shrink-0">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="4" d="M15 19l-7-7 7-7"></path></svg>
            </a>
            <div class="w-10 h-10 md:w-12 md:h-12 bg-[#FF3EA5] rounded-full flex items-center justify-center text-white font-black border-2 border-[#FF3EA5] shrink-0 uppercase">
                {{ substr($activeDoctor->name, 0, 1) }}
            </div>
            <div class="min-w-0">
    {{-- Nama Dokter --}}
    ...
    <div id="chat-scroll-area">
        <template x-for="msg in messages">
            <div class="flex flex-col w-full animate-fadeIn" :class="msg.sender === 'user' ? 'items-end' : 'items-start'">
                ...
                <span class="text-[9px] text-[#FF3EA5] mt-1.5 font-black uppercase opacity-40 px-2" x-text="msg.time"></span>
            </div>
        </template>
    </div>
    ...
    <div class="p-4 md:p-6 bg-white border-t-4 border-[#FF3EA5] shrink-0">
        <div class="flex items-end gap-4 max-w-7xl mx-auto">
            <div class="flex-1">
                <textarea 
                    x-model="messageInput" x-ref="input" rows="1" @input="resizeInput"
            ...
            <button type="button" @click="sendAndReset()"
                ...
                <svg class="w-6 h-6 rotate-90" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg>
            </button>
        </div>
...
<script>
    function chatSystem() {
        return {
            messageInput: '', messages: [], doctorId: null,
            initChat(docId) {
                this.doctorId = docId;
                this.fetchMessages();
            ...
            sendMessage() {
                ...
                this.messages.push({ text: text, sender: 'user', time: '...' });
                this.messageInput = ''; this.resizeInput();
                this.$nextTick(() => this.scrollToBottom());
                fetch('/tanya-dokter/send', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                    body: JSON.stringify({ doctor_id: this.doctorId, message: text })
                }).then(() => this.fetchMessages());
            },
            sendAndReset() { this.sendMessage(); },
            resizeInput() {
